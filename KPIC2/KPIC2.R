...
kpic_pattern <- function(data, file_save_path){
  labels <- c(rep('leaf',10), rep('seed',10))
  analyst.OPLS(labels, data$data.mat)
  write.csv(data$data.mat, file=file_save_path,
            quote=T, row.names = T)
}

kpic_DeepPIC <- function(pics5_ffn, pics5_dir, pics5_path1, pics5_ps, pics4_ffn, pics4_dir, pics4_path2, pics4_ps, pics_1_ff, pics_1_fps, pics_1_fp){
